   ...
   modewibox:setup({
         {
            ...
            id="margin",
            margins=beautiful.modebox_margin or dpi(7),
            color=beautiful.modebox_bg or
               beautiful.bg_normal,
...
local function show_box(s, map, name)
   ...
   local mar = mbox:get_children_by_id("border")[1]
   ...
   if settings.show_options then
      for _, mapping in ipairs(map) do
         ...
         elseif mapping[1] ~= "onClose" then
            label = label .. "\n<b>" .. awful.util.escape(mapping[1] == " " and "Space" or mapping[1]) ..
               "</b>\t\t" .. (mapping[3] or "???")
         end
      end
   ...
   local text_margin = (beautiful.modebox_margin or dpi(7))

   local x, y = txt:get_preferred_size(s)
   mbox.width = x + mar.left + mar.right + (text_margin*2)
   mbox.height = math.max(settings.height, y + mar.top + mar.bottom + (text_margin*2))
